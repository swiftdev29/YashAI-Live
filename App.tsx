    ...
    <div className="min-h-screen bg-[#0A0A0A] text-white font-sans flex flex-col relative overflow-hidden selection:bg-blue-500/30">
      ...
      <header className="absolute top-0 w-full p-6 z-20 flex justify-between items-center">
        <div className="flex items-center gap-3">
           ...
           <span className="font-semibold tracking-wider text-sm text-slate-200/90 font-mono uppercase text-shadow-sm">YashAI Voice</span>
        </div>
      ...
      {/* Grounding / Search Sources Panel - Restored and Enhanced */}
      {groundingMetadata && groundingMetadata.groundingChunks && groundingMetadata.groundingChunks.length > 0 && (
          <div 
            key={groundingMetadata.groundingChunks[0]?.web?.uri || 'grounding'}
            className="absolute bottom-10 left-1/2 -translate-x-1/2 w-full max-w-2xl px-4 z-30"
          >
              <div className="bg-black/40 backdrop-blur-2xl border border-white/10 rounded-2xl p-4 shadow-[0_0_50px_rgba(0,0,0,0.5)] animate-slide-up transform-gpu">
                      ...
                      {groundingMetadata.groundingChunks.map((chunk, idx) => (
                          chunk.web?.uri && (
                              <a 
                                  ...
                                  className="flex-shrink-0 flex flex-col bg-white/5 hover:bg-blue-500/10 border border-white/5 hover:border-blue-500/30 rounded-xl px-4 py-3 transition-all min-w-[200px] max-w-[240px] group snap-start"
                              >
                                  <span className="text-xs text-slate-100 font-medium line-clamp-2 w-full group-hover:text-blue-300 transition-colors h-8">
                                  ...
                                  <div className="flex items-center gap-1.5 mt-2">
                                      ...
                                      <span className="text-[9px] text-slate-500 font-mono truncate w-full uppercase">
                ...
                >
                    <div className="relative">
                        <input 
                            ...
                            className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3.5 text-center text-white placeholder-slate-600 focus:outline-none focus:border-blue-500/50 transition-all text-sm tracking-widest"
                            autoFocus
                        />
                    </div>
                    ...
                    <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium py-3.5 rounded-xl transition-all shadow-lg shadow-blue-500/20 mt-2">Authenticate</button>
                </form>
            </div>
            <div className="absolute bottom-8 text-[10px] text-slate-700 font-mono tracking-widest uppercase">Authorized Access Only</div>
        </div>
    );
  }
